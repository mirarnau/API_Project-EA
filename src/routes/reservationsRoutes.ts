...
class ReservationsRoutes {
  ...
  public async addReservation (req: Request, res: Response) : Promise<void> {
    ...
    const {
      _idCustomer, _idRestaurant, nameCustomer, nameRestaurant, numCustomers, phone, dateReservation, timeReservation
    } = req.body
  ...
  public async deleteReservation (req: Request, res: Response) : Promise<void> {
    ...
    const customer = await Customer.findById({ _id: reservationToDelete._idCustomer })
    if (customer == null) {
      res.status(404).send('Customer not found.')
      return
    }
